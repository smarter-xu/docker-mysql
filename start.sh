...
if [ ! -d /var/lib/mysql/mysql ]; then
  ...
  echo "Creating smarter user..."
  mysql -uroot -e "GRANT ALL PRIVILEGES on *.* TO 'smarter'@'%' IDENTIFIED BY 'smarter' WITH GRANT OPTION;"

  echo "Creating repl user..."
  mysql -uroot -e "GRANT REPLICATION SLAVE, REPLICATION CLIENT on *.* TO 'repl'@'%' IDENTIFIED BY 'repl' WITH GRANT OPTION;"
